	...
	<div class="index-wrap">
		<div class="index-left">
			<div class="index-left-block">
				<h2>全部产品</h2>
				<template v-for="prodauct in productList">
					<h3>{{ prodauct.title }}</h3>
					<ul>
						<li v-for="item in prodauct.list">
							<a :href="item.url">{{ item.name }}</a>
							<span v-if="item.hot" class="hot-tag">HOT</span>
						</li>
					</ul>
					<div v-if="!prodauct.last" class="hr"></div>
				</template>
			</div>
...
export default {
	components: {
		...
		created : function () {
			var that = this
			this.$http.get("http://127.0.0.1:3000/api/getNewsList")
			.then(function(res){
